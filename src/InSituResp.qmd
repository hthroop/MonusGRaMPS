...
csv_files <- list.files(here("data", "L1_InSituRespiration"), pattern = "\\.csv$", full.names = TRUE)
...
## Reduce Data to Means

```{r}
#| label: daily collar means

# calculate collar-level means for replicate respiration measurements per collar (~ 4 reps per collar) for each date x plot x site combination x treatmenet x microsite combination
...
```{r}
#| label: microsite x plot means for each trip

# calculate means for each microsite x plot x site x trip combination 
InSituResp_microsite_means <- InSituResp_daily_collar_means |>
  group_by(site, Treatment, Microsite, Season, Trip) |>
  summarise(
    N_plots     = n(),
    Efflux_micro   = mean(Efflux, na.rm = TRUE),
    sd_Efflux_micro   = sd(Efflux, na.rm = TRUE),
    se_Efflux_micro   = sd_Efflux_micro / sqrt(N_plots),
    RHcmbr   = mean(RHcmbr, na.rm = TRUE),
    Tsoil_C_micro   = mean(Tsoil_C, na.rm = TRUE),
    sd_Tsoil_C_micro = sd(Tsoil_C, na.rm=TRUE),
    se_Tsoil_C_micro  = sd_Tsoil_C_micro / sqrt(N_plots),
    Tsch_C   = mean(Tsch_C, na.rm = TRUE),
    .groups = "drop"
  )
```
