...
Canexsituresp <- ExSituRespRates |>
  filter(Canopy == "Can") |>
  filter(CO2flux > 0)

# explore outliers
hist(Canexsituresp$CO2flux) # nearly all data are below 150 for flux
hist(Canexsituresp$Cum.Elapsed.days) # most data are 100 days or less, a few outliers around 200 days
...
# remove outliers where resp > 75 and time > 100 days
CanexsiturespNoOutliers <- subset(Canexsituresp, CO2flux<75 & Cum.Elapsed.days<100) 

# save clean data
write.csv(CanexsiturespNoOutliers, here("results", "CanexsiturespNoOutliers.csv"))
```

```{r}


ggplot(CanexsiturespNoOutliers, aes(x = Cum.Elapsed.days, y = CO2flux, color = Sample, group = Sample)) +
  geom_line(size = 1) +
  facet_grid(Treatment ~ Site) +
  labs(
    x = "Time",
    y = expression(CO[2]~Flux~(µg~C~g^{-1}~hr^{-1})),
    title = "CO₂ Flux over Time by Site and Treatment",
    color = "Sample"
  ) +
  theme_minimal() +
  theme(
